...
import { Mail, Github, FileText, CheckCircle, ExternalLink, BookOpen } from 'lucide-react';
...
export default function Contact() {
  ...
  return (
    <div id="contact" ref={containerRef} className="min-h-screen bg-[#F5F5F7] text-[#1D1D1F] selection:bg-blue-100 overflow-hidden relative flex items-center justify-center py-24 md:py-32 lg:py-40">
      
      {/* 1. 배경 요소: CONTACT 텍스트 & 컬러 블롭 */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <h1 className="absolute top-[10%] left-1/2 -translate-x-1/2 text-[20vw] font-black text-black/[0.03] select-none tracking-tighter">
          CONTACT
        </h1>
      ...
      {/* 2. 연결 선 (Hero로부터 카드 상단으로 이어짐) */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1px] h-40 bg-gradient-to-b from-transparent to-[#007AFF]"></div>
      <div className="absolute top-40 left-1/2 -translate-x-1/2 w-8 h-8 rounded-full border border-[#007AFF]/30 flex items-center justify-center">
      ...
      <section className="relative z-10 w-full max-w-[1440px] px-8">
          ...
          <div className="grid lg:grid-cols-10 items-stretch min-h-[600px] md:min-h-[700px]">
            ...
            <div className="lg:col-span-6 p-10 md:p-16 lg:p-20 flex flex-col justify-center border-b lg:border-b-0 lg:border-r border-white/20">
              <div className="contact-left-animate flex items-center gap-3 mb-8">
                <span className="w-12 h-[1px] bg-[#007AFF]"></span>
                <span className="text-[#007AFF] font-bold tracking-[0.3em] text-sm uppercase">Get in Touch</span>
              </div>
            ...
            <div className="contact-cards-container lg:col-span-4 p-8 md:p-12 bg-white/5 flex flex-col justify-center">
                ...
                {contactLinks.map((link, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-5">
                      <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${link.color} flex items-center justify-center text-white shadow-lg shadow-blue-500/10 group-hover:scale-110 transition-transform duration-500`}>
                        {link.icon}
                      </div>
                      <div>
                        <h4 className="text-lg font-bold text-[#1D1D1F] group-hover:text-[#007AFF] transition-colors tracking-tight">{link.name}</h4>
                        <p className="text-xs text-[#86868B] font-medium leading-none mt-1">{link.desc}</p>
                      </div>
                    </div>
              ...
              {/* 하단 추가 안내 */}
              <div className="contact-card-animate mt-12 text-center lg:text-left px-4">
                <p className="text-[#86868B] text-sm font-medium leading-relaxed opacity-70">
                  새로운 기회와 성장을 위한 연결을 환영합니다. <br/>
                  기획의 진심을 기술의 가치로 구현합니다.
                </p>
              </div>
            </div>
        ...
        <div className="mt-12 flex flex-col items-center">
          <div className="w-[1px] h-16 bg-gradient-to-b from-[#007AFF] to-transparent mb-8 opacity-30"></div>
          <p className="text-[#A1A1A6] text-xs font-bold tracking-[0.4em] uppercase">
            © 2026 Hyunwook. All rights reserved.
          </p>
        </div>
      </section>
    </div>
