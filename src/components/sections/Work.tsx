      ...
      <div className="max-w-7xl mx-auto  relative z-11">
        ...
        >
          ...
          <p className="text-gray-500 max-w-2xl mx-auto">
            41건 이상의 프로젝트 데이터 중 핵심적인 성과를 선별했습니다.
            <br className="hidden md:block" />각 카테고리를 클릭하여 현욱님의
            전문성을 확인해 보세요.
          </p>
        </motion.div>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {categories.map((cat) => (
            <button
              ...
              className={`px-6 py-2 rounded-full text-sm font-bold transition-all border cursor-pointer ${
                activeCategory === cat
                    ...
                    <div className="translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                      ...
                      <div className="flex flex-wrap gap-2 mb-6 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
                        {project.tags.map((tag, i) => (
                          <span
                            key={i}
                            className="text-[10px] bg-white/10 backdrop-blur-md px-2 py-1 rounded border border-white/10"
                          >
                            #{tag}
                          </span>
                        ))}
        ...
        >
          <div className="flex flex-col md:flex-row items-center justify-center gap-12">
            <div>
              <div className="text-5xl font-black text-brand-blue mb-2 font-en">
                41+
              </div>
